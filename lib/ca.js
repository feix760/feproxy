...
const untrustRootCA = getRootCA('feproxy.untrust');
const trustedRootCA = getRootCA('feproxy');

function getRootCA(name) {
  const keyFile = path.join(__dirname, `../run/${name}.key`);
  const certFile = path.join(__dirname, `../run/${name}.crt`);
...
function getCertificate(hostname, isTrust = true) {
  return createCertificate(isTrust ? trustedRootCA : untrustRootCA, hostname);
}
