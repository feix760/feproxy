...
- Adding a plugin = a new file in `src/proxy/` + registration in `proxyPlugins.ts` (priority / default match) + a new branch in the `to` string encode/decode of the frontend's `component/Project.tsx`.
