...
Basic proxy auth with credentials **hardcoded** in `auth` in `defaultConfig.ts` (default `feproxy/feproxy`, `enable: false`). `ProxyConfig.update()` strips the `auth` field, so it can't be changed through `/setConfig`. Both config endpoints answer through `publicConfig()` in `controller/site.ts`, a **whitelist** of what the admin page reads (`projects/https/ignoreCertError/inspect` plus `auth: { enable }`, and the derived `devtoolsURL`/`crtURL` on `getConfig`) — so credentials, `RC_DIR`, `hostname`, `port` and the rule cache stay on the server, and the page shows auth as a disabled switch. **Extend that whitelist when the frontend starts reading a new field.**
