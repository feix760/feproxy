...
export default (app: FeproxyApp) => {
  ...
  Object.defineProperties(app.context, {
    ...
    routerPath: {
      ...
      // @koa/router writes back ctx.routerPath = layer.path after matching; swallow it to keep
      // the getter (its dist is use strict, so a getter-only property would throw a TypeError)
      set() {},
    },
    // @koa/router picks the path to match in the order
    // `opts.routerPath || ctx.newRouterPath || ctx.path || ctx.routerPath`.
    // ctx.path is pathname only, and coming before routerPath it would break every absolute-URL
    // route, so we feed the absolute URL in through the highest-priority newRouterPath.
    newRouterPath: {
      get(this: ProxyContext) {
